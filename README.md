...
## 核心功能
### ✅ 已完成的 MVP 功能
- **工作流引擎**：基于状态机的 WorkflowExecutor，支持多步骤编排、触发检测、审批恢复执行
- **审批系统**：多级审批链、同步/并行审批、角色权限控制
- **AI 集成**：DeepSeek API 集成，AI 辅助内容生成和初审
- **模板系统**：预设模板（客服工单审批流、数据录入审批流、内容发布审批流）
- **飞书集成**：审批通知推送飞书，机器人交互
- **仪表盘**：执行指标、成功率、趋势图（Recharts）
- **审计日志**：完整的操作追溯
- **用户系统**：NextAuth 认证 + 定价页面
- **单元测试**：Vitest 框架 + 22+ 测试用例
- **重试/超时**：API 调用重试机制、超时处理
- **部署就绪**：Dockerfile、Vercel 配置
